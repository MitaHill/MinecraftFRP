...
def run_cli(servers, args):
    ...
    if server_name not in servers:
        logger.error(f"错误: 无效的服务器名称 '{server_name}'。可用服务器: {', '.join(servers.keys())}")
        sys.exit(1)

    local_port = determine_local_port(args)
    if not local_port:
        logger.error("错误: 必须提供 --local_port 或 --auto-find 并成功检测到端口")
        sys.exit(1)
...
def determine_local_port(args):
    ...
    if args.auto_find:
        ...
        if port:
            logger.info(f"检测到Minecraft游戏端口: {port}")
            return port
        else:
            logger.info("未检测到Minecraft游戏端口")
            if not args.local_port:
                return None
    
    if args.local_port:
        if not args.local_port.isdigit() or not 1 <= int(args.local_port) <= 65535:
            logger.error("错误: 请输入有效端口 (1-65535)")
            sys.exit(1)
...
def start_mapping_cli(server_name, local_port, servers):
    ...
    if not config_manager.create_config(host, port, token, local_port, remote_port, user_id):
        logger.error("错误: 无法写入配置文件，请检查权限")
        sys.exit(1)

    link = f"{host}:{remote_port}"
    logger.info(f"开始映射本地端口 {local_port} 到 {link}")

    th = FrpcThread("config/frpc.ini")
    th.out.connect(lambda msg: logger.info(msg))
    th.warn.connect(lambda msg: logger.warning(f"警告: {msg}"))
    th.success.connect(lambda: logger.info(f"映射成功！映射地址: {link}"))
    th.error.connect(lambda msg: (logger.error(f"错误: {msg}"), sys.exit(1)))
    th.terminated.connect(lambda: (logger.info("frpc进程已终止"), sys.exit(0)))
    th.start()
